      ...
      steps{
          ...
          //   sh 'ssh -i  /root/.ssh/"sandbox-env.pem"  ubuntu@ec2-18-204-4-100.compute-1.amazonaws.com'
            ...
            sshagent(credentials : ['login_sandbox_server	']) {
             ...
             sh 'ssh -o StrictHostKeyChecking=no ubuntu@ec2-18-204-4-100.compute-1.amazonaws.com "echo pwd && sudo -i -u root && docker -v && docker login -u ${{DOCKERHUB_CREDENTIALS.DOCKERHUB_CREDENTIALS_USR}} -p ${{DOCKERHUB_CREDENTIALS.DOCKERHUB_CREDENTIALS_PSW}} '
